...
- 每个函数、方法、构造函数、回调和 React 组件都必须有贴近定义位置的注释；测试中的辅助函数与回调同样适用。
- 函数注释应说明职责、关键输入或输出、可观察副作用及重要约束；不要复述函数名或类型已表达的信息。
- 重要、分支较多或不易直观理解的逻辑必须补充行内注释，说明业务原因、关键不变量或边界条件，而非逐行翻译代码。
- 修改既有代码时，应同时补齐受影响函数和相关复杂逻辑的缺失注释，并保持注释与实现同步。
