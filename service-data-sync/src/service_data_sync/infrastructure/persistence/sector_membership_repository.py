"""使用 `PostgreSQL` 保存板块成分观察快照、半开区间和固定 `release` 清单。

来源只提供完整观察集合，区间边界表达“连续两次观察之间未见变化”，不是官方纳入或剔除
生效日。每个板块、市场日和 `schema` 构成可重试请求键；固定 `release manifest` 让读者
可以复验某一消费者版本所使用的完整成员集合，而不依赖随后变化的当前快照。
"""
