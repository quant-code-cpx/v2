...
class EquityListingStatusVersion(Base):
    """记录上市、暂停、退市及官方更正的双时态生命周期证据。

    状态只能由专用生命周期来源发布；目录、行情或同步缺席都不能自动产生退市结论。业务有效区间
    说明事实何时成立，知识区间说明平台何时接受该证据；若官方更正历史状态，追加新版本并关闭旧
    知识区间。状态反转必须附带受控原因和来源血缘，避免把临时数据质量问题写成证券生命周期。
    """
