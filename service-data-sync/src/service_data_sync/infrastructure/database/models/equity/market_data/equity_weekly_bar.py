...
class EquityWeeklyBar(Base):
    ...
    __table_args__ = (
        ...
        ),
        {
            ...
            "comment": "上游直取的未复权个股周线 revision；按 period_end 年度分区。",
        },
    )
    ...
    period_end: Mapped[date] = mapped_column(
        Date, primary_key=True, nullable=False, comment="上游周线周期结束日。"
    )
    revision: Mapped[int] = mapped_column(
        Integer, primary_key=True, nullable=False, comment="同一证券周期的递增修订号。"
    )
    open_price: Mapped[Decimal] = mapped_column(
        Numeric(20, 6), nullable=False, comment="上游周线直接提供的开盘价。"
    )
    high_price: Mapped[Decimal] = mapped_column(
        Numeric(20, 6), nullable=False, comment="上游周线直接提供的最高价。"
    )
    low_price: Mapped[Decimal] = mapped_column(
        Numeric(20, 6), nullable=False, comment="上游周线直接提供的最低价。"
    )
    close_price: Mapped[Decimal] = mapped_column(
        Numeric(20, 6), nullable=False, comment="上游周线直接提供的收盘价。"
    )
    volume_shares: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="上游周线成交量，统一为股。"
    )
    amount_cny: Mapped[Decimal] = mapped_column(
        Numeric(24, 4), nullable=False, comment="上游周线成交额，单位人民币元。"
    )
    turnover_rate: Mapped[Decimal | None] = mapped_column(
        Numeric(16, 10), nullable=True, comment="上游周线换手率，统一为小数。"
    )
    is_final: Mapped[bool] = mapped_column(
        Boolean, nullable=False, comment="该周线是否已经形成完整周期。"
    )
    content_sha256: Mapped[bytes] = mapped_column(nullable=False, comment="周线业务内容稳定摘要。")
    source_batch_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("source_batch.source_batch_id", ondelete="RESTRICT"),
        nullable=False,
        comment="支撑该周线 revision 的独立来源观察。",
    )
