"""按证券与 `provider-neutral capability` 独立维护的已成功市场数据增量检查点模型。"""
...
class EquitySyncCheckpoint(Base):
    ...
    日线、周线、月线、复权因子、公司行动和资料的可恢复位置各自不同；即使日期看似相同，也不能
    用一个周期成功来证明另一个周期已抓取。检查点只在对应内容成功规范化并发布后更新，失败批次
    保留错误和来源证据但不得推进它，防止后续任务跳过未完成窗口。
    """
