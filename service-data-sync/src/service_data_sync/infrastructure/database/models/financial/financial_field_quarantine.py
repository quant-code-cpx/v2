...
class FinancialFieldQuarantine(Base):
    ...
    上游新增字段、字段类型漂移或语义无法确认时，服务必须显式隔离而非把值塞进通用 JSON 或猜测
    映射到已治理指标。记录通过摘要、批次和字段路径支持排障，同时避免把受限 raw 内容复制进
    数据库；处置完成后需发布受控字典/映射版本，不能直接让历史隔离项自动变成可见事实。
    """
