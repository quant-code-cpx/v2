...
class FinancialStatementFact(Base):
    ...
    每行关联明确的报表 `revision` 和指标字典，数值、单位、币种及其空值原因一起表达来源事实。
    零、未披露、不适用、来源未知和多币种绝不是同一语义，不能为方便计算而互相转换；未知字段
    应进入隔离表。物理分区跟随报告期，读取仍必须经已发布版本确保报表和行项目处于同一知识截点。
    """
