...
def _client() -> tuple[TestClient, dict[str, str], FakeMarketDataRepository]:
    ...
    app = create_app(
        ...
        market_data_repository=cast(MarketDataAccessRepository, repository),
    )
    token = settings.internal_api_bearer_token.get_secret_value()
    return TestClient(app), {"Authorization": f"Bearer {token}"}, repository
