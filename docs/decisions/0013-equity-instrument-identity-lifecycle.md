...
AKShare 1.18.78 只是候选采集库，不是生产许可或稳定性保证。2026-07-27 的 Docker 探针确认：
...
这些结果只证明一次接口形状和失败模式。来源许可、长期稳定性、交易所日历和字段法律语义仍待评审。
...
- 本 ADR 不批准任何来源用于生产。生产启用前必须完成交易所/聚合源许可、调用频率、原始留存、
  字段语义和连续稳定性评审。
